...
[Files]
; 预编译的插件 DLL（AnyCPU：32 位/64 位 SolidWorks 都能加载）
Source: "bin\AnyCPU\Release\SWAIAddin.dll"; DestDir: "{app}"; Flags: ignoreversion
...
; 普通电脑只有 Runtime 没有 SDK，RegAsm.exe 不存在 → COM 注册悄悄失败 → 插件列表不显示。
